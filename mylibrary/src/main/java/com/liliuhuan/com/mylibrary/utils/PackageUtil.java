...
public class PackageUtil {
	...
	public static String getAppPackageName(){
		String pName = LibraryApp.getContext().getPackageName();
		return pName;
	}

	/** 根据packageName获取packageInfo */
	public static PackageInfo getPackageInfo(String packageName) {
		Context context = LibraryApp.getContext();
		if (null == context) {
			return null;
		}
	...
	/** 比较检查的版本号是否比当前更高, 1 = 当前版本更高，-1 = 当前版本更低，0 = 相同 */
	public static int compareVersionCode(String strNewVersion) {
		String strCurVersion = getVersionName();
		String [] curVersion = strCurVersion.split("\\.");
		String [] newVersion = strNewVersion.split("\\.");
		int nIndex = 0;

		while(true) {
			if(curVersion.length <= nIndex) {
				if(newVersion.length > nIndex) {
					return -1;
				}
				return 0;
			}
			...
			if(Integer.parseInt(curVersion[nIndex]) > Integer.parseInt(newVersion[nIndex])) {
				return 1;
			} else if(Integer.parseInt(curVersion[nIndex]) < Integer.parseInt(newVersion[nIndex])) {
				return -1;
			}
			++nIndex;
		}
	...
	/** 获取本应用的VersionName　versionCode+versionName组成 */
	...
	public static String getChannelName(String name){
		...
		String value = null;
		try {
			appInfo = LibraryApp.getContext().getPackageManager().getApplicationInfo(LibraryApp.getContext().getPackageName(),PackageManager.GET_META_DATA);
