 ...
 * @author KingKong-HE
 * @Time 2015-11-13
 * @Email kingkonghe@vip.qq.com
 */
public class StringWheelView extends View {
    /**
     * 控件宽度
     */
     ...
     * 单击事件
     */
    private OnWheelViewOnClickListener onClickListener;
    /**
     * 是否可用
     */
                    ...
                    for (int i = itemList.size() - 1; i >= 0; i--) {
                        if (itemList.get(i).isSelected()) {
                            if (onClickListener != null) {
                                onClickListener.OnClick(itemList.get(i));
                                break;
                            }
                        }
                    }
        ...
        for (ItemObject item : itemList) {
            if (item.isSelected()) {
                if (onSelectListener != null) {
                    onSelectListener.endSelect(item);
                }
                break;
            }
        }
        ...
        if (move > 0) {
            for (int i = 0; i < itemList.size(); i++) {
                if (itemList.get(i).isSelected()) {
                    ...
                    if (onSelectListener != null)
                        onSelectListener.endSelect(itemList.get(i));
                    break;
                }
            }
        } else {
    ...
    public OnWheelViewOnClickListener getWheelViewOnClickListener() {
        return onClickListener;
    }

    public void setWheelViewOnClickListener(OnWheelViewOnClickListener onClickListener) {
    ...
    private void onSelectListener() {
        ...
        for (ItemObject item : itemList) {
            if (item.isSelected()) {
                onSelectListener.selecting(item);
            }
        }
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        unitHeight = (int) attribute.getDimension(
                R.styleable.WheelView_unitHight, UIUtil.dip2px(48));
        normalFont = attribute.getDimension(
                R.styleable.WheelView_normalTextSize, UIUtil.dip2px(16));
        selectedFont = attribute.getDimension(
                R.styleable.WheelView_selectedTextSize,UIUtil.dip2px(18));
        itemNumber = attribute.getInt(R.styleable.WheelView_itemNumber, 4);
        normalColor = attribute.getColor(
                R.styleable.WheelView_normalTextColor, 0xff777777);
        selectedColor = attribute.getColor(
                R.styleable.WheelView_selectedTextColor, 0xff000000);
        lineColor = attribute.getColor(R.styleable.WheelView_lineColors,
                0xCDCDCDCD);
        lineWidth = attribute.getDimension(R.styleable.WheelView_lineHeight, 1.5f);
    ...
    public interface OnSelectListener {
         ...
         * 结束选择
         */
        public void endSelect(ItemObject itemObject);

        /**
         * 选中的内容
         */
        public void selecting(ItemObject itemObject);

    }

    public interface OnWheelViewOnClickListener {
